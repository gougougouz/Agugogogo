...
def generate_finetune_data(similar_questions,prices, dates, finetune_data_name):
    ...
    for i in range(len(prices) - 23):
        ...
        # 将日期和股价拼接成字符串
        input_data_str = ', '.join([f"day{date}: {price}" for date, price in zip(segment_dates[:16], segment_prices[:16])])
        output_data_str = ', '.join([f"day{date}: {price}" for date, price in zip(segment_dates[16:], segment_prices[16:])])
    ...
    # 保存为 JSON 文件
    with open(f'{finetune_data_name}_finetune_data.json', 'w', encoding='utf-8') as f:
        json.dump(dataset, f, ensure_ascii=False, indent=4)

def split_dataset(file_path, train_ratio=0.8, output_dir="/home/ubuntu/PEFT/小作坊/"):
    """
    ...
    # 构建输出文件路径
    train_file_path = os.path.join(output_dir, f"train_{file_base_name}{file_extension}")
    val_file_path = os.path.join(output_dir, f"val_{file_base_name}{file_extension}")
...
# 数据切分
file_path = r"/home/ubuntu/PEFT/dataset/002475_2020-01-01_2023-12-01.csv"
split_dataset(file_path=file_path)

#微调数据集构造
train_file_path=r"/home/ubuntu/PEFT/小作坊/train.csv"
finetune_data_name=build_finetune_dataset_name(file_path)
close_prices=read_xlsx_colum(file_path=file_path,column=3)
