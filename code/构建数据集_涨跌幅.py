...
def build_finetune_dataset_name(file_path):
    ...
    # 构造数据集名字
    finetune_data_name = stock_code + "_"
...
def generate_finetune_data(similar_questions,prices, dates, finetune_data_name):
    ...
    # 保存为 JSON 文件
    with open(f'pct_finetune_data/{finetune_data_name}_finetune_data.json', 'w', encoding='utf-8') as f:
        json.dump(dataset, f, ensure_ascii=False, indent=4)

def split_dataset(file_path, train_ratio=0.8, train_output_dir="训练集/train_pct/",val_output_dir="验证集/val_pct/"):
    """
...
train_data=get_all_files_absolute_path(r"D:/a股是我们的/训练集/")
for file in train_data:
    file_path = file
    #微调数据集构造
    # train_file_path=r"数据集/train.csv"
    finetune_data_name=build_finetune_dataset_name(file_path)
    pct=read_xlsx_colum(file_path=file_path,column=12)
    dates=read_xlsx_colum(file_path=file_path,column=0)
    similr_questions=read_similar_questions("相似问法_涨跌幅.txt")
    generate_finetune_data(similr_questions,pct,dates,finetune_data_name)
